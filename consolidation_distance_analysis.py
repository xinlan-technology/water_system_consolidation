...
def create_consolidation_sensitivity_plot():
    ...
    try:
        ...
        ax.plot(sensitivity_df['Distance_km'], sensitivity_df['Joint_Merger'], 'o-', 
                label='Joint Merger', color='blue', linewidth=2, markersize=4)
        ax.plot(sensitivity_df['Distance_km'], sensitivity_df['Balanced_Merger'], 's-', 
                label='Balanced Merger', color='green', linewidth=2, markersize=4)
        ax.plot(sensitivity_df['Distance_km'], sensitivity_df['Direct_Acquisition'], '^-', 
                label='Direct Acquisition', color='red', linewidth=2, markersize=4)
        ax.plot(sensitivity_df['Distance_km'], sensitivity_df['Total_Consolidation'], 'D-', 
                label='Total Consolidation', color='purple', linewidth=3, markersize=5)
